...
| 11 | Implement user-type specific views: end-users see only menu options for their role and can only view/update records allowed by their privileges | **Done** | `Layout.jsx` filters nav by role; `ProtectedRoute` + `allowedRoles`; manager-only prioritization via `requireCpscManager`; admin-only `/admin/users`. |
| 12 | Application compatible across multiple devices (desktop, mobile, tablet) | **Done** | **Layout**: mobile drawer + desktop sidebar (existing). **Recalls**: horizontal scroll on wide table, full-width search on small screens, form `maxWidth` breakpoints. Spot-check on real devices still useful. |
...
|-----------|--------|-------|
| Screens match Requirements + UX designs | **In progress** | Main app is React+MUI under `client/`; separate Figma/Make under `Wireframes for Recall Management System (Community)/` — align sprint screens deliberately. |
